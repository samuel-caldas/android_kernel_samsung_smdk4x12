 ...
 * do loopback mount.
 */
static int do_loopback(struct path *path, const *old_name,
				int recurse)
{
	LIST_HEAD(umount_list);
...
static int do_move_mount(struct path *path, const *old_name)
{
 ...
 * namespace's tree
 */
static int do_new_mount(struct path *path, const *type, int flags,
			int mnt_flags, const *name, void *data)
{
	struct vfsmount *mnt;
	int err;
